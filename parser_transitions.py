...
def minibatch_parse(sentences, model, batch_size):
    ...
    # Keep track of which parses are unfinished (shallow copy of partial_parses)
    unfinished_parses = partial_parses[:]
    ...
    while unfinished_parses:
        ...
        # Predict the next transitions for the minibatch
        transitions = model.predict(minibatch)

        # Apply each transition to the corresponding partial parse
        for i, transition in enumerate(transitions):
            minibatch[i].parse_step(transition)

        # Remove completed parses (those with empty buffer and stack having only "ROOT")
        unfinished_parses = [pp for pp in unfinished_parses if len(pp.buffer) > 0 or len(pp.stack) > 1]

    # Return the dependencies in the same order as the sentences
